package com.github.mrgalaxy.bitmodule.lib;

import src.run.Fear_Atlas;
import src.run.GameVersion;

/**
 * BitModule.
 * 
 * Engine Client Reference Class
 * 
 * @author Mr_Galaxy
 * @license Lesser GNU Public License v3 (http://www.gnu.org/licenses/lgpl.html)
 * 
 */

public class Reference
{
	//public static final int WIDTH = 300;
	//public static final int HEIGHT = WIDTH / 16 * 9; // Calculation done in Main class 168px
	//public static final int scale = 3;
    
    public static final String DISPLAY = "Display";
	public static final String ENGINE_NAME = "BitModule";
    public static final String ENGINE_LONG_NAME = "BitModule Game Engine";
	public static final String TITLE = Fear_Atlas.NAME + " " + GameVersion.GAME_VERSION;

	public static final String BUILD_DEV = "!!- DEV-VERSION -!!";
    public static final String IS_RUNNING = "Running...";
    public static final String IS_STOPING = "Stoping...";
    public static final String BUILD_SYMBOL = Strings.ALPHA;
    
    public static final String string = "Hello";
    
}